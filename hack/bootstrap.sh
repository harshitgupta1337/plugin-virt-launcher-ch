...
function kubevirt::bootstrap::regenerate() {
    (
        ...
        # TODO KUBEVIRT_BOOTSTRAPPING=true bazel run --config=${ARCHITECTURE} //:gazelle -- -exclude vendor/google.golang.org/grpc --exclude kubevirtci/cluster-up
        # TODO KUBEVIRT_BOOTSTRAPPING=true bazel run --config ${HOST_ARCHITECTURE} //rpm:sandbox_${1}
        bazel clean
...
function kubevirt::bootstrap::sha256() {
    (
        cd ${KUBEVIRT_DIR}
        # TODO sha256sum rpm/BUILD.bazel | head -c 40
    )
}
