...
import (
	...
	agentpoller "github.com/harshitgupta1337/plugin-virt-launcher-ch/pkg/virt-launcher-cloud-hypervisor/virtwrap/agent-poller"
	...
	"github.com/harshitgupta1337/plugin-virt-launcher-ch/pkg/virt-launcher-cloud-hypervisor/virtwrap/converter/arch"
	"github.com/harshitgupta1337/plugin-virt-launcher-ch/pkg/virt-launcher-cloud-hypervisor/virtwrap/efi"
	"github.com/harshitgupta1337/plugin-virt-launcher-ch/pkg/virt-launcher-cloud-hypervisor/virtwrap/testing"
...
var _ = Describe("Manager", func() {
	...
	Context("on successful VirtualMachineInstance sync", func() {
		It("should define and start a new VirtualMachineInstance", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			mockLibvirt.ConnectionEXPECT().DomainDefineXML(string(xml)).DoAndReturn(mockDomainWithFreeExpectation)
		...
		It("should not add discard=unmap if a disk is preallocated", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			mockLibvirt.ConnectionEXPECT().DomainDefineXML(gomock.Any()).DoAndReturn(func(xml string) (cli.VirDomain, error) {
				By(fmt.Sprintf("%s\n", xml))
		...
		Context("Memory hotplug", func() {
			...
			It("should update the virtio-mem device if it already exists", func() {
				...
				size, err := api.QuantityToByte(resource.MustParse("128Mi"))
				Expect(err).ToNot(HaveOccurred())
				requested, err := api.QuantityToByte(resource.MustParse("64Mi"))
				Expect(err).ToNot(HaveOccurred())
				block, err := api.QuantityToByte(resource.MustParse("2Mi"))
				Expect(err).ToNot(HaveOccurred())
	...
	Context("on successful VirtualMachineInstance migrate", func() {
		...
		It("should verify that migration failure is set in the monitor thread", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			mockLibvirt.DomainEXPECT().GetJobStats(libvirt.DomainGetJobStatsFlags(0)).AnyTimes().Return(fake_jobinfo, nil)
			mockLibvirt.DomainEXPECT().GetXMLDesc(gomock.Any()).AnyTimes().Return(string(domainXml), nil)

			mockLibvirt.DomainEXPECT().MigrateToURI3(gomock.Any(), gomock.Any(), gomock.Any()).Return(fmt.Errorf("MigrationFailed"))
			options := &cmdclient.MigrationOptions{
				Bandwidth:               resource.MustParse("64Mi"),
				ProgressTimeout:         150,
				CompletionTimeoutPerGiB: 300,
